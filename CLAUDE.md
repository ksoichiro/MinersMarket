...
The 1.21.1 structure files are converted to 1.20.1 format during the 1.20.1 build by a Groovy/Java converter (the former Python script is gone). `buildSrc` compiles the shared Querz-NBT converter classes (srcDir'd from `gradle/shared/src/main/groovy`, dependency `com.github.Querz:NBT:6.1` via jitpack), and `common/1.20.1/build.gradle` registers a `convertNbt` task that runs `com.github.ksoichiro.mcmod.V1_21ToV1_20NbtConverter`. `buildSrc` is used deliberately: applying the shared mcmod Gradle plugin (or a per-module buildscript classpath dependency) conflicts with Fabric Loom's classloader, whereas `buildSrc` does not diverge any project's buildscript classloader and is therefore Loom-safe.
