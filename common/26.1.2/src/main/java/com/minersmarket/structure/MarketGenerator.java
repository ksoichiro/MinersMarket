...
public class MarketGenerator {
    private static final Identifier TEMPLATE_ID =
            Identifier.fromNamespaceAndPath(MinersMarket.MOD_ID, "market");
    ...
    public static boolean generate(ServerLevel level, BlockPos center) {
        ...
        tmpl.placeInWorld(level, placePos, placePos, settings, level.getRandom(), BLOCK_UPDATE_FLAGS);
        ...
        level.setRespawnData(net.minecraft.world.level.storage.LevelData.RespawnData.of(level.dimension(), spawnPos, 0, 0));

        // Set spawn radius to 0 so players spawn exactly at the set position
        level.getGameRules().set(net.minecraft.world.level.gamerules.GameRules.RESPAWN_RADIUS, 0, level.getServer());
    ...
    private static int findPlacementY(ServerLevel level, int x, int z) {
        // MC 1.21.9 removed spawn chunks, so chunks may not be loaded when the
        // market generates at SERVER_STARTED. Force chunk loading to ensure the
        // heightmap is populated; otherwise getHeight returns getMinY() (-64).
        level.getChunk(net.minecraft.core.SectionPos.blockToSectionCoord(x),
