...
public class GameStartBlock extends Block {
    ...
    protected InteractionResult useWithoutItem(BlockState state, Level level, BlockPos pos,
        ...
        if (manager.isCountdownActive()) {
            serverPlayer.sendSystemMessage(Component.translatable("message.minersmarket.countdown_in_progress"));
        } else if (manager.getState() == GameState.IN_PROGRESS) {
            serverPlayer.sendSystemMessage(Component.translatable("message.minersmarket.already_in_progress"));
        } else if (manager.getState() == GameState.ENDED) {
            serverPlayer.sendSystemMessage(Component.translatable("message.minersmarket.game_ended_reset_required"));
        }
